...
class weighted_digraph:
    ...
    def dijkstra(self, start):
        ...
        while todo:
            min = None
            for node in todo:
                if min is None or node.distance < min.distance:
                    min = node
            ...
            for edge in min.edges:
                if edge.to_node.distance == float("inf") or edge.to_node.distance > min.distance + edge.weight:
                    edge.to_node.distance = min.distance + edge.weight
                    edge.to_node.previous = min
                    todo.add(edge.to_node)

        for i in result:
            i.extend(self.track_path(i[1]))
        return result

    def track_path(self, end):
        lastNode = self.find(end)
        reversePath = []
        while lastNode.previous is not None:
            reversePath.append(lastNode.previous.value)
...
class test_weighted_digraph(unittest.TestCase):
    ...
    def test_track_path(self):
        ...
        g.add_edges([(1, 2, 2), (1, 3, 1), (2, 3, 1), (2, 4, 1), \
                     (2, 5, 2), (3, 5, 5), (4, 5, 3), (4, 6, 6), (5, 6, 1)])
        g.dijkstra(1)
        self.assertEqual(g.track_path(6), [5, 2, 1])
